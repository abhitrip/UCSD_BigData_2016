...
# your code here
new_data = text_rdd.filter(lambda x: safe_parse(x)==True)         .map(lambda x: (ujson.loads(x)))          .map(lambda x: (x['user']['id_str'],x['text'])) 
...
tokenUsersRDD = new_data.flatMapValues(lambda x : tok.tokenize(x)).distinct()#@abhi
...
#tfm = tokensUsersRDD.flatMapValues(lambda x:x).distinct()# @abhijit
tfm = tokenUsersRDD#@abhijit.distinct()
...
"""
"""
tokenMappedUsers = tokenUsersRDD.map(gid_subs)
temp = tokenMappedUsers.map(lambda x:(x,1))                        .reduceByKey(lambda x,y :x+y)                         .filter(lambda(x,y) : x[1] in commonTokens )                        .map(lambda (x,y) : (x[0],(x[1],y)))
...
temp1 = temp.mapValues(calcRelFreq).groupByKey().mapValues(lambda x:[a for a in x])                    .mapValues(lambda k :sorted(k,key= lambda y:(-y[1],y[0])))                    .collectAsMap()  
...
for idx,item in enumerate(temp1):
    print_tokens(temp1[idx][0:10],idx)
